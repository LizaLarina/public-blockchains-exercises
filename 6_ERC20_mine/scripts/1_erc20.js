...
const cAddress = "0x347bb91e00EdbC585df75D69E58A661E23a8F949";
const cName = "MyERC20";
...
const mint = async (amount) => {
    ...
    // Your code here.
    const contract = await getContract(signer);
    let tx = await contract.mint(signer.address, amount);
    await waitForTx(tx);
    const newTotalSupply = Number(await contract.totalSupply());
    console.log('New total supply:', newTotalSupply);

    let balance = await contract.balanceOf(deployer.address);
    console.log("Current deployer balance: ", Number(balance));
    let balanceReceiver = await contract.balanceOf(signer.address);
    console.log("Current signer balance: ", Number(balanceReceiver));
